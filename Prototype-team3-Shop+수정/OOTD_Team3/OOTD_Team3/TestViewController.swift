...
class TestViewController: UIViewController {
    ...
    private func loadQuestion() {
        ...
        UIView.animate(withDuration: 0.5) { [weak self] in
            self?.progressBarFill.widthAnchor.constraint(equalTo: self?.progressBar.widthAnchor ?? NSLayoutDimension(), multiplier: progress).isActive = true
            self?.progressBarFill.layoutIfNeeded()
        }
